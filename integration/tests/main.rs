...
fn test_end_to_end_migration_non_public_ext_schema() -> Result<()> {
    ...
    let ext_exists_sql = "SELECT EXISTS (
        SELECT
			1
        FROM
            pg_extension
        ...
        WHERE
            extname = 'timescaledb'
			AND nspname = 'CustomSchema'
	)";

    source_assert.is_true(ext_exists_sql);
    target_assert.is_true(ext_exists_sql);
...
#[test]
fn test_end_to_end_migration_non_public_ext_schema_on_cloud() -> Result<()> {
    let _ = pretty_env_logger::try_init();
    ...
    // Make target container a cloud setup.
    psql(
        &target_container,
        Sql(r"
		INSERT INTO _timescaledb_catalog.metadata(key, value, include_in_telemetry) VALUES
			('forge_profile_id', 'dev', 't'),
			('forge_project_id', 'abcd', 't'),
			('forge_service_id', 'efgh', 't');
	"),
    )?;

    // On source, extension must exist in CustomSchema.
    source_assert.is_true(
        "SELECT EXISTS (
        SELECT
			1
        FROM
    ...
    // On target, extension must exist in public schema.
    target_assert.is_true(
        "SELECT EXISTS (
        SELECT
			1
        FROM
    ...
    let snapshot = RunnableImage::from((
        ...
        vec![
            ...
            String::from("--force-timescaledb-public-schema"),
        ],
    ))
    .with_network(&network_name);
    let _snapshot = docker.run(snapshot);

...
typedef struct SourceMaterializedViewArrayContext
{
	char sqlstate[SQLSTATE_LENGTH];
	DatabaseCatalog *catalog;
	bool parsedOk;
} SourceMaterializedViewArrayContext;
...
static void getMaterializedViewArray(void *ctx, PGresult *result);
 ...
 * For code simplicity the index array is also the SourceFilterType enum value.
 */
struct FilteringQueries listSourceMaterializedViewSQL[] = {
	{
		SOURCE_FILTER_TYPE_NONE,
...
/*
 * schema_list_materialized_view grabs the list of materialized view refreshes from the
 * given source Postgres instance and allocates a SourceTable array with
 * the result of the query.
 */
bool
schema_list_materialized_views(PGSQL *pgsql,
							  SourceFilters *filters,
							  DatabaseCatalog *catalog)
{
	SourceMaterializedViewArrayContext context = { { 0 }, catalog, false };

	log_trace("schema_list_materialized_views");
	...
	log_debug("listSourceMaterializedViewSQL[%s]",
			filterTypeToString(filterType));

	char *sql = listSourceMaterializedViewSQL[filterType].sql;

	if (!pgsql_execute_with_params(pgsql, sql, 0, NULL, NULL,
								   &context, &getMaterializedViewArray))
	{
...
/*
 * getTableArray loops over the SQL result for the materialized view refresh
 * array query and allocates an array of materialized view then populates it
 * with the query result.
 */
static void
getMaterializedViewArray(void *ctx, PGresult *result)
{
	SourceMaterializedViewArrayContext *context = (SourceMaterializedViewArrayContext *) ctx;
	...
	for (int rowNumber = 0; rowNumber < nTuples; rowNumber++)
	{
		SourceMaterializedView mv = { 0 };

		/* c.oid */
		char *value = PQgetvalue(result, rowNumber, fnoid);

		if (!stringToUInt32(value, &(mv.oid)) || mv.oid == 0)
		{
			log_error("Invalid OID \"%s\"", value);
			++errors;
		}
		...
		int length = strlcpy(mv.nspname, value, PG_NAMEDATALEN);
		...
		length = strlcpy(mv.relname, value, PG_NAMEDATALEN);
		...
		mv.excludeData = (*value) == 't';

		/* restoreListName */
		value = PQgetvalue(result, rowNumber, fnrestorelistname);
		length = strlcpy(mv.restoreListName, value, RESTORE_LIST_NAMEDATALEN);
		...
		if (context->catalog != NULL && context->catalog->db != NULL)
		{
			if (!catalog_add_s_matview(context->catalog, &mv))
			{
				++errors;
